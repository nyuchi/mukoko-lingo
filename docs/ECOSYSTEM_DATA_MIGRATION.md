...
**Status:** complete. All six phases have shipped: `identity.persons` cutover (PR #24), `DB_NAME` fixed from the invented `mukoko-lingo` to the real `lingo` database, phrases repointed at `lingo.phrases` (PR #25), guardrails repointed at `shamwari.guardrails` (PR #24), Shamwari AI conversations moved to `shamwari.conversations`/`shamwari.messages` (PR #29), bookmarks/views wired up as plain `lingo` collections with a live engagement view (PR #28), XP synced into `ubuntu.contributions` (PR #27), and API keys cut over to `platform.apiKeys` (PR #30). `classes`/`assignments`/`srs_cards`/`user_skills`/`assessments` remain intentionally Lingo-local — no ecosystem equivalent exists for them.
...
### Phase 3 — Shamwari AI conversations — **done**

`api/ai/conversations/*` now read/write the real `shamwari.conversations`/`shamwari.messages` collections (`surfaceContext: 'mukoko-lingo'`), replacing the local `ai_conversations` collection entirely — `ObjectId` dropped in favor of UUID string `_id`s, messages are their own collection rather than a 200-message-capped embedded array. Every `shamwari.conversations` document requires an `ownerEntityId`; since per-person "family" entity auto-provisioning (Rule 10) isn't implemented anywhere in the ecosystem yet, `resolveOwnerEntityId()` falls back to the `MUKOKO_LINGO_ENTITY_ID` product entity when a person has no `bundu.defaultFamilyEntityId` — a known, explicitly-flagged simplification, not a bug. External API contract (`aiApi` client) unchanged.
...
### Phase 5 — Ubuntu gamification (XP/streaks/leaderboard) — **done**

`POST /api/xp` now mirrors every synced XP event into `ubuntu.contributions` (`sourceDomain: "lingo"`, `category: "cultural"` — the closest fit in the existing enum, since nothing maps cleanly onto "language learning"), 1:1 with `xp_events` (not batched), as a best-effort side write that never blocks the user's XP sync. `contributorEntityId` uses the same `defaultFamilyEntityId` → `MUKOKO_LINGO_ENTITY_ID` fallback as Phase 3. **Still blocked, unchanged from before**: no Ubuntu-backed leaderboard or badges/missions — `ubuntu.leaderboardDefinitions`/`badges`/`missions` remain empty ecosystem-wide (re-confirmed live), so Lingo's existing local weekly leaderboard (`api/leaderboard/index.ts`) stays the real, working feature.

### Phase 6 — API keys — **done**

`api/admin/api-keys/*` now read/write the shared `platform.apiKeys` collection (`keyType: 'external'`, `surfaceContext: 'lingo'`) instead of the local, never-populated `api_keys` collection. Key hashing uses `scrypt` (not a bare digest) so the check doesn't trip CodeQL's insufficient-computational-effort rule, even though the underlying key is already high-entropy. Endpoint access is restricted to platform admins (`requireAdmin`) — the original design's "org admin" concept (a `classMemberships` teacher role) had no relationship to the target `organization_id` and would have let any teacher self-service-issue a key for an arbitrary organization; there's no ecosystem-wide "org admin of a specific entity" concept yet to check against instead.
