-- ============================================================================
-- NYUCHI LINGO - COMPLETE DATABASE REBUILD V2
...
-- ============================================================================
-- CLEANUP: Remove existing tables if they exist (CAREFUL!)
-- ============================================================================
-- Comment this section out if you want to keep existing data

DROP TABLE IF EXISTS moderation_alerts CASCADE;
...
-- Trigger to create profile on new user signup (only if auth.users exists)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'auth' AND table_name = 'users') THEN
...
BEGIN
  ...
  RAISE NOTICE '✅ 5 skills seeded';
  RAISE NOTICE '✅ 25 skill levels seeded (5 per skill)';
  RAISE NOTICE '✅ 5 learning standards seeded';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Next: Build skills-aware AI prompt system';
END $$;
